...
const CommentsPage = () => {
  ...
  const handleReport = async (commentId) => {
    ...
    if (!feedback || feedback === "Select Feedback") {
      // Check for empty or default feedback
            ...
            <tbody>
              {comments.map((comment) => {
                ...
                const isReported =
                  ...
                  comment.reported; // Check if already reported

                return (
                  <tr key={comment._id} className="align-top">
                    <td className="min-w-[120px]">{comment.userEmail}</td>{" "}
                    {/* Display userEmail */}
